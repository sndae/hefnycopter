...
volatile uint8_t m1,m2,m3,m4,m5;
	
uint16_t tempTCNT1;
uint8_t tempTCNT2;

uint8_t ServoRefresh=0;
...
void output_motor_ppm(void)
{
	...
	ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
   {
	...
	////}
		...
		if ( MotorOut5 < 0 ) m5 = 0;
		else if ( MotorOut5 > MOTORS_HIGH_VALUE ) m5 = MOTORS_HIGH_VALUE;
		else m5 = MotorOut5;
	...
	ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
   {
	...
	{
		while (TCNT2 < 1) //tempTCNT2);
		//for (j=0;j<3;++j);
		// NO CODE SHOULD BE ADDED HERE . IT IS ECAXCTLY 4 us loop
		if (i==m1) M1 = 0; //else M6 = 0;
		if (i==m2) M2 = 0; //else M6 = 0;
		if (i==m3) M3 = 0; //else M6 = 0;
		if (i==m4) M4 = 0; //else M6 = 0;
		if (i==m5) M5 = 0; //else M6 = 0;
